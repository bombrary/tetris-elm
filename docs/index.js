...
var $author$project$Board$height = 22;
...
var $author$project$Board$indexToVec = function (i) {
	return {
		x: A2($elm$core$Basics$modBy, $author$project$Board$width, i),
		y: (i / $author$project$Board$width) | 0
	};
};
...
var $author$project$Mino$Info = F5(
	function (rotMax, color, p1, p2, p3) {
		return {color: color, p1: p1, p2: p2, p3: p3, rotMax: rotMax};
	});
var $author$project$Color$LightBlue = {$: 'LightBlue'};
...
var $author$project$Mino$info = function (mino) {
	switch (mino.$) {
		case 'I':
			return A5(
				$author$project$Mino$Info,
				4,
				$author$project$Color$LightBlue,
...
var $author$project$Vec$rotate90 = function (_v0) {
	var x = _v0.x;
	var y = _v0.y;
	return {x: y, y: -x};
};
...
var $author$project$Main$elapseTime = function (model) {
	return _Utils_update(
		model,
		{time: model.time + 1});
};
...
var $author$project$Board$chunksOf = F2(
	function (n, list) {
		var _v0 = A2($elm$core$List$take, n, list);
		if (!_v0.b) {
			return _List_Nil;
		} else {
			var xs = _v0;
...
var $author$project$Board$trueIndicies = function (list) {
	return A2(
		$elm$core$List$map,
		function (_v1) {
			var i = _v1.a;
...
var $author$project$Board$filledLines = function (board) {
	return $author$project$Board$trueIndicies(
		A2(
			$elm$core$List$map,
			$elm$core$List$all(
				function (c) {
					return !_Utils_eq(c, $author$project$Board$Empty);
				}),
			A2($author$project$Board$chunksOf, $author$project$Board$width, board)));
};
var $author$project$Board$indexedFilter = F2(
	function (pred, list) {
		return A2(
			$elm$core$List$map,
			function (_v1) {
...
var $author$project$Board$moveOver = F2(
	function (lines, board) {
		return _Utils_ap(
			A2(
				$elm$core$List$repeat,
				$elm$core$List$length(lines) * $author$project$Board$width,
				$author$project$Board$Empty),
			A2(
				$author$project$Board$indexedFilter,
				F2(
					function (i, _v0) {
						return !A2($elm$core$List$member, (i / $author$project$Board$width) | 0, lines);
					}),
				board));
	});
var $author$project$Board$eraseLines = function (board) {
	var lines = $author$project$Board$filledLines(board);
	var erasedLines = A2($author$project$Board$moveOver, lines, board);
	return _Utils_Tuple2(
		erasedLines,
		$elm$core$List$length(lines));
};
var $author$project$Main$eraseLines = function (model) {
	var board = model.board;
	var lines = model.lines;
	var _v0 = $author$project$Board$eraseLines(board);
	var newBoard = _v0.a;
	var erasedLines = _v0.b;
	return _Utils_update(
		model,
		{board: newBoard, lines: lines + erasedLines});
};
var $author$project$Main$moveIfPossible = F4(
	function (dx, dy, board, state) {
		var pos = state.pos;
...
var $author$project$Main$fallDown = function (model) {
	...
	return (!A2($elm$core$Basics$modBy, 15, time)) ? _Utils_update(
		model,
		{
			minoState: A2(
				$elm$core$Maybe$map,
...
var $author$project$Main$setLifeTime = F2(
	function (t, state) {
		return _Utils_update(
			state,
			{
				lifeTime: $elm$core$Maybe$Just(t)
			});
	});
var $author$project$Main$startLifeTime = function (state) {
	return _Utils_update(
		state,
		{
			lifeTime: $elm$core$Maybe$Just(20)
		});
};
var $author$project$Main$fixMinoIfPossible = function (model) {
	var minoState = model.minoState;
	var board = model.board;
	if (minoState.$ === 'Just') {
		var state = minoState.a;
		var newState = _Utils_update(
			state,
			{
				pos: A2(
			...
			if (_v2.$ === 'Just') {
				if (!_v2.a) {
					return _Utils_update(
						model,
						{
							board: A2($author$project$Main$maskMinoIfPossible, minoState, board),
							minoState: $elm$core$Maybe$Nothing
						});
				} else {
					var t = _v2.a;
					return _Utils_update(
						model,
						{
							minoState: $elm$core$Maybe$Just(
								A2($author$project$Main$setLifeTime, t - 1, state))
						});
				}
			} else {
				return _Utils_update(
					...
					{
						minoState: $elm$core$Maybe$Just(
							$author$project$Main$startLifeTime(state))
					});
			}
		} else {
			return model;
...
var $author$project$Mino$getKickList = F3(
	function (o1, o2, mino) {
		switch (mino.$) {
			case 'I':
				return A2($author$project$Mino$getKickListI, o1, o2);
			case 'O':
				return _List_fromArray(
					[
						A2($author$project$Vec$Vec, 0, 0)
...
var $author$project$Main$tryKickList = F4(
	function (kickList, dr, board, state) {
		tryKickList:
		while (true) {
			if (!kickList.b) {
				return state;
				...
				if (_v1.$ === 'Just') {
					var $temp$kickList = ks,
						...
						$temp$board = board,
						$temp$state = state;
					kickList = $temp$kickList;
					dr = $temp$dr;
					board = $temp$board;
					state = $temp$state;
					continue tryKickList;
				} else {
					return newState;
				}
			}
...
var $author$project$Main$rotateIfPossible = F3(
...
var $author$project$Main$initMinoState = function (mino) {
	return {
		lifeTime: $elm$core$Maybe$Nothing,
		mino: mino,
		pos: {x: 4, y: 0},
		rot: 0
	};
};
var $author$project$MinoQueue$insertToNext = F2(
	function (list, q) {
		return _Utils_update(
			q,
			{next: list});
	});
var $author$project$Mino$I = {$: 'I'};
var $author$project$Mino$J = {$: 'J'};
var $author$project$Mino$L = {$: 'L'};
var $author$project$Mino$O = {$: 'O'};
var $author$project$Mino$S = {$: 'S'};
var $author$project$Mino$T = {$: 'T'};
var $author$project$Mino$Z = {$: 'Z'};
...
var $author$project$Mino$randoms = $elm_community$random_extra$Random$List$shuffle(
	_List_fromArray(
		[$author$project$Mino$I, $author$project$Mino$L, $author$project$Mino$J, $author$project$Mino$Z, $author$project$Mino$S, $author$project$Mino$O, $author$project$Mino$T]));
var $author$project$MinoQueue$swap = function (q) {
	var now = q.now;
	var next = q.next;
	return _Utils_update(
		q,
...
var $author$project$MinoQueue$takeOne = function (q) {
	var now = q.now;
	if (!now.b) {
		return $elm$core$Maybe$Nothing;
	} else {
...
var $author$project$Main$nextMino = function (model) {
	var minoQueue = model.minoQueue;
	var minoState = model.minoState;
	if (!model.gameOver) {
		if (minoState.$ === 'Just') {
			return model;
		} else {
			var _v1 = $author$project$MinoQueue$takeOne(minoQueue);
			if (_v1.$ === 'Nothing') {
				var _v2 = A2($elm$random$Random$step, $author$project$Mino$randoms, model.seed);
				var minos = _v2.a;
				var newSeed = _v2.b;
				var newQueue = A2(
					$author$project$MinoQueue$insertToNext,
					minos,
					$author$project$MinoQueue$swap(minoQueue));
				return _Utils_update(
					model,
					{minoQueue: newQueue, seed: newSeed});
			} else {
				var _v3 = _v1.a;
				var mino = _v3.a;
				var newQueue = _v3.b;
				return _Utils_update(
					model,
					{
						minoQueue: newQueue,
						minoState: $elm$core$Maybe$Just(
							$author$project$Main$initMinoState(mino))
					});
			}
		}
	} else {
...
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'Tick':
				return _Utils_Tuple2(
					$author$project$Main$checkGameOver(
						$author$project$Main$nextMino(
							$author$project$Main$eraseLines(
								$author$project$Main$fixMinoIfPossible(
									$author$project$Main$fallDown(
...
var $author$project$Main$boardWidth = 10;
var $author$project$Main$cellLength = 20.0;
...
var $author$project$Main$tetrisSideWidth = 80.0;
var $rtfeldman$elm_css$Css$displayFlex = A2($rtfeldman$elm_css$Css$property, 'display', 'flex');
var $author$project$Main$boardHeight = 20;
...
var $author$project$Board$clip = F2(
	function (rows, board) {
		return A2($elm$core$List$drop, $author$project$Board$width * rows, board);
	});
var $rtfeldman$elm_css$Svg$Styled$g = $rtfeldman$elm_css$Svg$Styled$node('g');
var $author$project$Vec$mult = F2(
	function (c, _v0) {
		var x = _v0.x;
		var y = _v0.y;
		return {x: c * x, y: c * y};
	});
var $author$project$Vec$toFloat = function (_v0) {
	var x = _v0.x;
	var y = _v0.y;
	return {x: x, y: y};
};
var $author$project$Board$toPositions = function (board) {
	return A2(
		$elm$core$List$indexedMap,
		F2(
			function (i, c) {
				return {
					pos: $author$project$Board$indexToVec(i),
					val: c
				};
			}),
		board);
...
var $author$project$Color$toCode = function (color) {
	switch (color.$) {
		case 'Purple':
			return '#75507b';
		case 'LightBlue':
...
var $author$project$Main$square = F3(
	function (len, sColor, fColor) {
		return A2(
			...
			_List_fromArray(
				[
					...
					$author$project$Color$toCode(sColor)),
					$rtfeldman$elm_css$Svg$Styled$Attributes$fill(
					$author$project$Color$toCode(fColor))
				]),
			_List_Nil);
	});
var $author$project$Main$viewCell = function (cell) {
	if (cell.$ === 'Empty') {
		return A3($author$project$Main$square, $author$project$Main$cellLength, $author$project$Color$Gray, $author$project$Color$White);
	} else {
		var col = cell.a;
		return A3($author$project$Main$square, $author$project$Main$cellLength, $author$project$Color$Gray, col);
	}
};
			...
			_List_fromArray(
				[
					...
					$author$project$Main$translate(
						{x: $author$project$Main$cellLength, y: 0.0}))
				]),
			A2(
				$elm$core$List$map,
				function (_v0) {
					...
					return A2(
						...
						_List_fromArray(
							[
								...
								$author$project$Main$translate(
									A2(
										$author$project$Vec$mult,
										$author$project$Main$cellLength,
										$author$project$Vec$toFloat(pos))))
							]),
						_List_fromArray(
							[
								$author$project$Main$viewCell(val)
							]));
				},
				$author$project$Board$toPositions(
					A2(
						$author$project$Board$clip,
						$author$project$Board$height - $author$project$Main$boardHeight,
						A2($author$project$Main$maskMinoIfPossible, state, board)))));
	});
var $author$project$Color$Black = {$: 'Black'};
var $author$project$Main$viewHorizontalWall = function (y) {
	return A2(
		$rtfeldman$elm_css$Svg$Styled$g,
		_List_fromArray(
			[
				$rtfeldman$elm_css$Svg$Styled$Attributes$transform(
				$author$project$Main$translate(
					{x: $author$project$Main$cellLength, y: $author$project$Main$cellLength * y}))
			]),
		A2(
			$elm$core$List$map,
			function (x) {
				return A2(
					...
					_List_fromArray(
						[
							$rtfeldman$elm_css$Svg$Styled$Attributes$transform(
							$author$project$Main$translate(
								{x: $author$project$Main$cellLength * x, y: 0}))
						]),
					_List_fromArray(
						[
							A3($author$project$Main$square, $author$project$Main$cellLength, $author$project$Color$Black, $author$project$Color$Black)
						]));
			},
			A2($elm$core$List$range, 0, $author$project$Main$boardWidth - 1)));
};
var $author$project$Main$viewVerticalWall = function (x) {
	return A2(
		$rtfeldman$elm_css$Svg$Styled$g,
		_List_fromArray(
			[
				$rtfeldman$elm_css$Svg$Styled$Attributes$transform(
				$author$project$Main$translate(
					{x: $author$project$Main$cellLength * x, y: 0.0}))
			]),
		A2(
			$elm$core$List$map,
			function (y) {
				return A2(
					...
					_List_fromArray(
						[
							$rtfeldman$elm_css$Svg$Styled$Attributes$transform(
							$author$project$Main$translate(
								{x: 0, y: $author$project$Main$cellLength * y}))
						]),
					_List_fromArray(
						[
							A3($author$project$Main$square, $author$project$Main$cellLength, $author$project$Color$Black, $author$project$Color$Black)
			...
			A2($elm$core$List$range, 0, $author$project$Main$boardHeight)));
};
var $author$project$Main$viewWall = A2(
	$rtfeldman$elm_css$Svg$Styled$g,
	_List_Nil,
	_List_fromArray(
		[
			$author$project$Main$viewVerticalWall(0),
			$author$project$Main$viewVerticalWall($author$project$Main$boardWidth + 1),
			$author$project$Main$viewHorizontalWall($author$project$Main$boardHeight)
		]));
var $rtfeldman$elm_css$Css$width = $rtfeldman$elm_css$Css$prop1('width');
var $author$project$Main$viewTetrisBody = function (model) {
	return A2(
			...
			[
				...
				_List_fromArray(
					[
						...
						$rtfeldman$elm_css$Css$px($author$project$Main$cellLength * ($author$project$Main$boardWidth + 2))),
						$rtfeldman$elm_css$Css$height(
						$rtfeldman$elm_css$Css$px($author$project$Main$cellLength * ($author$project$Main$boardHeight + 1)))
					]))
			]),
		_List_fromArray(
			[
				A2($author$project$Main$viewBoard, model.minoState, model.board),
				$author$project$Main$viewWall
			]));
};
...
var $author$project$MinoQueue$take = F2(
	function (n, _v0) {
		var now = _v0.now;
		var next = _v0.next;
		return A2(
...
var $author$project$Main$cellIconLength = 10.0;
...
var $author$project$Main$minoIconHeight = 20.0;
var $rtfeldman$elm_css$Css$padding = $rtfeldman$elm_css$Css$prop1('padding');
var $rtfeldman$elm_css$Css$solid = {borderStyle: $rtfeldman$elm_css$Css$Structure$Compatible, textDecorationStyle: $rtfeldman$elm_css$Css$Structure$Compatible, value: 'solid'};
var $author$project$Mino$ViewBox = F4(
...
var $author$project$Vec$maximumX = A2(
	$elm$core$Basics$composeL,
...
var $author$project$Vec$maximumY = A2(
	$elm$core$Basics$composeL,
	$elm$core$List$maximum,
	$elm$core$List$map(
		function (_v0) {
...
var $author$project$Vec$minimumX = A2(
	$elm$core$Basics$composeL,
...
var $author$project$Vec$minimumY = A2(
	$elm$core$Basics$composeL,
	$elm$core$List$minimum,
	$elm$core$List$map(
		function (_v0) {
...
var $author$project$Mino$viewBox = function (mino) {
	var ps = $author$project$Mino$positions(
		$author$project$Mino$info(mino));
	var x0 = $author$project$Vec$minimumX(ps);
	var x1 = $author$project$Vec$maximumX(ps);
	var width = A3(
		$elm$core$Maybe$map2,
		F2(
			function (a, b) {
		...
		x0,
		x1);
	var y0 = $author$project$Vec$minimumY(ps);
	var y1 = $author$project$Vec$maximumY(ps);
	var height = A3(
		$elm$core$Maybe$map2,
		F2(
			function (a, b) {
	...
	return A2(
		$elm$core$Maybe$withDefault,
		A4($author$project$Mino$ViewBox, 0, 0, 0, 0),
		A5($elm$core$Maybe$map4, $author$project$Mino$ViewBox, x0, y0, width, height));
};
...
var $author$project$Main$viewMinoIcon = F2(
	function (isFirst, mino) {
		var vb = $author$project$Mino$viewBox(mino);
		var viewBoxVal = A2(
			$elm$core$String$join,
			' ',
			_List_fromArray(
				[
					$elm$core$String$fromFloat(vb.x * $author$project$Main$cellIconLength),
					$elm$core$String$fromFloat(vb.y * $author$project$Main$cellIconLength),
					$elm$core$String$fromFloat(vb.width * $author$project$Main$cellIconLength),
					$elm$core$String$fromFloat(vb.height * $author$project$Main$cellIconLength)
				]));
		var info = $author$project$Mino$info(mino);
		var positions = $author$project$Mino$positions(info);
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$rtfeldman$elm_css$Css$height(
							$rtfeldman$elm_css$Css$px($author$project$Main$minoIconHeight)),
							isFirst ? $rtfeldman$elm_css$Css$marginTop(
							$rtfeldman$elm_css$Css$px(10.0)) : $rtfeldman$elm_css$Css$marginTop(
							$rtfeldman$elm_css$Css$px(5.0)),
							$rtfeldman$elm_css$Css$padding(
							$rtfeldman$elm_css$Css$px(10.0)),
							isFirst ? A3(
							$rtfeldman$elm_css$Css$border3,
							$rtfeldman$elm_css$Css$px(1),
							$rtfeldman$elm_css$Css$solid,
							$rtfeldman$elm_css$Css$hex('#888')) : $rtfeldman$elm_css$Css$border($rtfeldman$elm_css$Css$zero)
						]))
				]),
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$rtfeldman$elm_css$Css$width(
									$rtfeldman$elm_css$Css$px(vb.width * $author$project$Main$cellIconLength)),
									$rtfeldman$elm_css$Css$height(
									$rtfeldman$elm_css$Css$px(vb.height * $author$project$Main$cellIconLength))
								])),
							$rtfeldman$elm_css$Svg$Styled$Attributes$viewBox(viewBoxVal)
						]),
					A2(
						$elm$core$List$map,
						function (pos) {
								...
								_List_fromArray(
									[
										...
										$author$project$Main$translate(
											A2(
												$author$project$Vec$mult,
												$author$project$Main$cellIconLength,
												$author$project$Vec$toFloat(pos))))
									]),
								_List_fromArray(
									[
										A3($author$project$Main$square, $author$project$Main$cellIconLength, $author$project$Color$Gray, info.color)
									]));
						},
						positions))
				]));
	});
var $author$project$Main$viewMinoQueue = function (model) {
	return A2(
		$rtfeldman$elm_css$Html$Styled$div,
		_List_fromArray(
			[
		...
		A2(
			...
			F2(
				function (i, x) {
					return A2($author$project$Main$viewMinoIcon, !i, x);
				}),
			A2($author$project$MinoQueue$take, 7, model.minoQueue)));
};
			...
			[
					...
					[
						...
						_List_fromArray(
							[
								...
								model.gameOver ? $rtfeldman$elm_css$Css$hex('#f00') : $rtfeldman$elm_css$Css$hex('#000')),
				...
				_List_fromArray(
					[
						...
						'Score: ' + $elm$core$String$fromInt(model.lines))
					]))
			]));
};
var $author$project$Main$viewTetrisSide = function (model) {
	return A2(
		$rtfeldman$elm_css$Html$Styled$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				$author$project$Main$viewMinoQueue(model)
			]));
};
var $author$project$Main$viewTetris = function (model) {
	return A2(
		$rtfeldman$elm_css$Html$Styled$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				$author$project$Main$viewTetrisBody(model),
				$author$project$Main$viewTetrisSide(model)
			]));
};
var $author$project$Main$viewTetrisDescription = function (model) {
	return A2(
		$rtfeldman$elm_css$Html$Styled$div,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$rtfeldman$elm_css$Html$Styled$text('←  → : Move horizontally')
							])),
						A2(
						$rtfeldman$elm_css$Html$Styled$p,
						_List_Nil,
						_List_fromArray(
							[
								$rtfeldman$elm_css$Html$Styled$text('↓ ↑: Soft and hard drop')
							])),
						A2(
						$rtfeldman$elm_css$Html$Styled$p,
						_List_Nil,
						_List_fromArray(
							[
								$rtfeldman$elm_css$Html$Styled$text('Z X: Rotate')
							]))
					]))
			]));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$rtfeldman$elm_css$Css$width(
								$rtfeldman$elm_css$Css$px(($author$project$Main$cellLength * ($author$project$Main$boardWidth + 2)) + $author$project$Main$tetrisSideWidth)),
								A4(
				...
				_List_fromArray(
					[
						$author$project$Main$viewTetris(model),
						$author$project$Main$viewTetrisDescription(model)
					]))
			]));
};
